      ...
      <div className="group overflow-y-auto custom-scrollbar max-h-[90vh]" onScroll={handleScroll}>
        {entry.imageUrl && (
          <div
            ...
            style={{ opacity: imageOpacity, transition: "opacity 0.1s ease-out" }}
          >
            <img
              src={entry.imageUrl}
              alt="Entry image"
...
export default function HomePage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (res.ok) {
      ...
      if (viewEntryId === entryToDelete.id) setViewEntryId(null);
    }
  ...
  return (
    <div className="flex min-h-screen bg-background pb-16 lg:pb-0">
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href}>
                <Button variant="ghost" className={`w-full justify-start gap-2 ${isActive ? "bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground" : ""}`}>
                    ...
                    {entry.imageUrl ? (
                      ...
                      <>
                        <div className="p-5">
                          ...
                          <p className="text-xs text-muted-foreground">{format(new Date(entry.createdAt), "MMMM d, yyyy")}</p>
      ...
      {isEditing && (
        // Wrap the editor in a div that stops event propagation
